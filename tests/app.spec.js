...
test('nav links are visible on home', async ({ page }) => {
  await page.goto('/')
...
test('studio page has all control buttons visible', async ({ page }) => {
  ...
  await expect(page.locator('.icon-btn')).toHaveCount(2)
  await expect(page.locator('.toggle')).toHaveCount(2)
})

test('studio page has 5 settings groups', async ({ page }) => {
  await page.goto('/')
  await page.locator('.btn-primary').click()
  await expect(page.locator('.setting-group')).toHaveCount(5)
})
...
test('footer credit is visible on home', async ({ page }) => {
  await page.goto('/')
  await expect(page.locator('.footer')).toContainText('Lumen')
})
